...
def task_row(conn: sqlite3.Connection, task_id: int) -> sqlite3.Row:
    row = conn.execute("SELECT t.*,u.name assignee_name,u.avatar_url assignee_avatar_url,r.name reviewer_name FROM tasks t LEFT JOIN users u ON u.id=t.assignee_id LEFT JOIN users r ON r.id=t.reviewer_id WHERE t.id=? AND t.deleted_at IS NULL", (task_id,)).fetchone()
    if not row:
