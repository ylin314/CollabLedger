...
function RecommendCard({ item, selected, onSelect, onAccept }) {
  ...
  return <div className={`recommend-item ${selected ? 'selected' : ''}`} onClick={() => onSelect(item.user_id)}>
    ...
    <div className="recommend-main">
      ...
      <p className="muted-note">{dims.skill?.note} · {sourceLabel(item.source || dims.skill?.source)}</p>
      {item.reasons?.evidence && <ul className="reason-evidence">{item.reasons.evidence.slice(0, 4).map(line => <li key={line}>{line}</li>)}</ul>}
    </div>
...
function RecommendationsView({ project, members, canWrite, onRecommend, onToast, setProject, onReload }) {
  ...
  return <>
    ...
    <div className="stage2-grid">
      ...
      <section className="panel"><div className="panel-header"><div><h2>最近推荐记录</h2><p>记录是否采纳、采纳了谁、是否改成别人</p></div></div>{(history?.items || []).length ? history.items.slice(0, 8).map(item => <div className="weekly-member-row" key={item.id}><div><strong>{item.task_name || '未命名任务'}</strong><span>{item.status_label || item.status} · {item.top?.name || '暂无候选'} {item.top?.score ? `· ${item.top.score}` : ''}</span></div><span>{item.created_at?.slice(0, 16)?.replace('T', ' ')}</span></div>) : <div className="empty-state">还没有推荐记录</div>}</section>
    </div>
