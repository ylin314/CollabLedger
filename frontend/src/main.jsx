...
function RecommendModal({ task, project, onClose, onToast, setProject }) {
  ...
  useEffect(() => { (async () => { try { const query = task.id ? new URLSearchParams({ task_id: String(task.id) }) : new URLSearchParams({ task_name: task.title, task_type: task.task_type || '', estimated_hours: task.estimated_hours || 1 }); const response = await getJson(`/api/projects/${project.id}/recommendations?${query}`); setResults(response.recommendations || []); setPayload(response) } catch (reason) { setError(reason.message) } finally { setBusy(false) } })() }, [])
  ...
  return <div className="modal-backdrop"><div className="modal recommend-modal"><div className="modal-head"><div><span className="eyebrow">AI RECOMMENDATION</span><h2>负责人建议</h2><p className="modal-sub">「{task.title}」 · 推荐仅供参考，不构成成员排名</p></div><button onClick={onClose}>×</button></div>{busy ? <div className="recommend-loading"><span className="loader"/>正在计算匹配度…</div> : error ? <div className="form-error">{error}</div> : <div className="recommend-list">{results.map((item, index) => <div className="recommend-item" key={item.user_id}><div className={`avatar avatar-${index % 5}`}>{initials(item.name)}</div><div className="recommend-main"><div className="recommend-name"><strong>{item.name}</strong><span className="score">匹配度 {Math.round(item.score)}<small>%</small></span></div><div className="reason-chips">{item.reasons?.summary && <span>{item.reasons.summary}</span>}<span>技能 {Math.round((item.reasons?.skill_match || 0) * 100)}%</span><span>质量 {item.reasons?.average_quality ?? '—'}/5</span><span>效率 {item.reasons?.efficiency ?? '—'}</span><span>负载 {item.reasons?.current_load || '—'}</span></div>{item.reasons?.evidence && <ul className="reason-evidence">{item.reasons.evidence.map(line => <li key={line}>{line}</li>)}</ul>}<div className="match-track"><i style={{ width: `${item.score}%` }}/></div></div><button className="choose-button" onClick={() => choose(item)}>采纳</button></div>)}{!results.length && <div className="empty-state">暂无可推荐成员{payload?.excluded_overloaded?.length ? `（已排除超负载 ${payload.excluded_overloaded.map(item => item.name).join('、')}）` : ''}</div>}</div>}<div className="recommend-foot"><span>◉ {payload?.disclaimer || '推荐仅供参考，最终由组长决定。'}</span><button className="ghost-button" onClick={onClose}>关闭</button></div></div></div>
}
