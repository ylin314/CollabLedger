...
function Overview({ auth, project, memberStats, tasks, progress, completed, overdue, activeTasks, online, risks, weekly, memberLoad, onNavigate, onAction, onRecommend, onProfile, canWrite, canManageTask }) {
  ...
  return <><PageTitle eyebrow={greetingStamp()} title={greetingTitle(auth?.name)} desc="这是你们小组本周的协作脉搏。推荐仅供参考，最终由组长决定。" action={canWrite ? <button className="primary-button" onClick={() => onNavigate('tasks')}><span>＋</span> 新建任务</button> : null}/><div className="overview-grid"><section className="hero-card"><div className="hero-copy"><span className="live-pill"><i/> 项目进行中</span><h2>{project.name}</h2><p>{project.description || '让每一份协作成果都被看见。'}</p><div className="hero-meta"><span>◷ {formatDate(project.start_date)} — {formatDate(project.end_date)}</span><span>♧ {memberStats.length} 位成员</span></div></div><div className="progress-ring" style={{ '--progress': `${progress * 3.6}deg` }}><div><strong>{progress}%</strong><span>项目进度</span></div></div></section>
    ...
    <div className="member-grid">{memberStats.map((m, i) => <MemberCard key={m.id} member={m} index={i} onProfile={onProfile}/>)}</div>
...
function MemberCard({ member, index, onProfile }) { const status = { online: ['协作中', 'online'], busy: ['专注中', 'busy'], away: ['暂离', 'away'], offline: ['离线', 'offline'] }[member.status] || ['协作中', 'online']; const loadLevel = member.load_level || (member.current >= member.max_concurrent_tasks ? 'high' : member.current / member.max_concurrent_tasks > 0.5 ? 'normal' : 'low'); return <div className="member-card"><div className="member-head"><div className={`avatar avatar-${index % avatarColors.length}`} style={{ background: avatarColors[index % avatarColors.length] }}>{initials(member.name)}</div><div><strong>{member.name}</strong><span className={`status ${status[1]}`}><i/>{status[0]}</span></div><span className={`load-pill ${loadLevel}`}>{member.load_label || ({ low: '低负载', normal: '正常', high: '高负载' }[loadLevel])}</span></div><div className="member-skills">{(member.skills || []).slice(0, 3).map(s => <span key={s}>{s}</span>)}</div><div className="member-foot"><span>当前任务 <b>{member.current}/{member.max_concurrent_tasks}</b></span><span>本周完成 <b>{member.done}</b></span><button className="profile-link" onClick={() => onProfile?.(member)}>画像</button></div><div className={`load-bar ${loadLevel}`}><i style={{ width: `${Math.min(100, member.current / Math.max(1, member.max_concurrent_tasks) * 100)}%` }}/></div></div> }
function ProfileModal({ user, onClose }) {
  const [profile, setProfile] = useState(null)
  const [error, setError] = useState('')
  useEffect(() => {
    let cancelled = false
    getJson(`/api/users/${user.id}/profile`).then(payload => { if (!cancelled) setProfile(payload) }).catch(reason => { if (!cancelled) setError(reason.message) })
    return () => { cancelled = true }
  }, [user.id])
  const families = profile?.skill_families || []
  const empty = profile && !families.length && !profile.quality_samples && !profile.efficiency_samples && !profile.contributions_total
  return <div className="modal-backdrop"><div className="modal profile-modal"><div className="modal-head"><div><span className="eyebrow">MEMBER PROFILE</span><h2>{profile?.name || user.name} 的历史画像</h2></div><button onClick={onClose}>×</button></div>
    {error ? <p className="profile-error">{error}</p> : !profile ? <div className="loading-state">正在聚合历史画像…</div> : empty ? <div className="empty-state">暂无历史画像</div> : <>
      <p className="profile-note">基于历史项目聚合，仅对本组成员可见；近 90 天样本权重 1.0，更早历史权重 0.5。</p>
      <div className="profile-grid">
        <div className="profile-skills"><h4>技能画像</h4>{families.length ? families.map(family => <SkillBar key={family.id} label={family.name} value={Math.round((profile.skill_strength?.[family.id] || 0) * 100)} color="purple"/>) : <p className="muted-note">暂无技能样本</p>}</div>
        <div className="profile-metrics">
          <div className="profile-metric"><strong>{profile.average_quality != null ? profile.average_quality : '—'}{profile.average_quality != null && <small> / 5</small>}</strong><span>平均质量（{profile.quality_samples} 条评价）</span></div>
          <div className="profile-metric"><strong>{profile.average_efficiency != null ? profile.average_efficiency : '—'}</strong><span>工时比（{profile.efficiency_samples} 条完成记录，越小越快）</span></div>
          <div className="profile-metric"><strong>{profile.contributions_total ?? 0}</strong><span>已确认贡献</span></div>
          <div className="profile-metric"><strong>{profile.projects_count ?? 0}</strong><span>参与项目数</span></div>
          <div className="profile-metric"><strong>{profile.active_months ?? 0}</strong><span>活跃月份</span></div>
        </div>
      </div>
    </>}
    <div className="modal-actions"><button className="ghost-button" onClick={onClose}>关闭</button></div>
  </div></div>
}
...
function sourceLabel(value) { return { rule: '规则', llm: 'AI 语义', embedding: '向量语义', hybrid: '规则 + AI' }[value] || value || '规则' }
function RecommendCard({ item, selected, onSelect, onAccept }) {
  ...
  return <div className={`recommend-item ${selected ? 'selected' : ''}`} onClick={() => onSelect(item.user_id)}>
    ...
    <div className="recommend-main">
      ...
      {item.reasons?.contrast && <p className="recommend-contrast">{item.reasons.contrast}</p>}
      ...
      <div className="reason-chips">{(dims.skill?.skill_families || []).slice(0, 3).map(family => <span key={family}>{family}</span>)}{dims.quality?.samples > 0 && <span>质量样本 {dims.quality.samples} 条</span>}{dims.efficiency?.samples > 0 && <span>工时样本 {dims.efficiency.samples} 条</span>}{dims.load?.current_load ? <span>负载 {dims.load.current_load}</span> : null}{item.profile_source === 'historical' && <span className="source-chip">参考历史画像</span>}<span className={`source-chip ${item.source !== 'rule' ? 'ai' : ''}`}>{sourceLabel(item.source || dims.skill?.source)}</span></div>
...
function RecommendModal({ task, project, members, onClose, onToast, setProject }) {
  ...
  return <div className="modal-backdrop"><div className="modal recommend-modal"><div className="modal-head"><div><span className="eyebrow">AI RECOMMENDATION</span><h2>负责人建议</h2><p className="modal-sub">「{task.title}」 · 推荐仅供参考，不构成成员排名，也不会自动指派</p></div><button onClick={onClose}>×</button></div>
    {busy ? <div className="recommend-loading"><span className="loader"/>正在计算匹配度…</div> : error ? <div className="form-error">{error}</div> : <>
      {payload?.comparison?.summary && <p className="recommend-summary">{payload.comparison.summary}</p>}
      <p className="muted-note">技能来源 {sourceLabel(payload?.skill_source)} · 理由来源 {sourceLabel(payload?.reason_source)} · 不会自动指派</p>
      {payload?.errors && Object.keys(payload.errors).length > 0 && <div className="degrade-note"><strong>⚠ AI 服务暂不可用，已自动回退规则计算</strong>{payload.errors.skill_error ? <span>技能：{payload.errors.skill_error}</span> : null}{payload.errors.reason_error ? <span>理由：{payload.errors.reason_error}</span> : null}</div>}
...
function RecommendationsView({ project, members, canWrite, onRecommend, onToast, setProject, onReload }) {
  ...
  return <>
    ...
    <div className="stage2-grid">
      ...
      <section className="panel"><div className="panel-header"><div><h2>最近推荐记录</h2><p>记录是否采纳、采纳了谁、是否改成别人</p></div></div>{(history?.items || []).length ? history.items.slice(0, 8).map(item => { const nameOf = id => (members || []).find(m => Number(m.user_id) === Number(id) || Number(m.id) === Number(id))?.name || '' ; return <div className="weekly-member-row" key={item.id}><div><strong>{item.task_name || '未命名任务'}</strong><span>{item.status_label || item.status} · 推荐 {item.top?.name || '暂无候选'}{item.top?.score ? `· ${item.top.score} 分` : ''}</span><span className="history-meta">{item.mode === 'batch' ? '批量生成' : '单任务生成'} · {sourceLabel(item.source)}{nameOf(item.accepted_user_id) ? ` · 采纳：${nameOf(item.accepted_user_id)}` : ''}{item.status === 'manual' ? ` · 改派给：${nameOf(item.assigned_user_id) || '其他成员'}` : ''}</span></div><span>{item.created_at?.slice(0, 16)?.replace('T', ' ')}</span></div> }) : <div className="empty-state">还没有推荐记录</div>}</section>
    </div>
    {(batch?.items || []).map(item => <section className="panel" key={item.recommendation_id}><div className="panel-header"><div><h2>{item.task.task_name}</h2><p>{item.source === 'rule' ? '规则推荐' : '规则 + AI 语义匹配'} · {item.disclaimer}</p></div></div>{item.comparison?.summary && <p className="recommend-summary">{item.comparison.summary}</p>}<div className="recommend-list">{(item.recommendations || []).map(candidate => <RecommendCard key={candidate.user_id} item={candidate} selected={false} onSelect={() => {}} onAccept={canWrite ? rec => decide(item.recommendation_id, rec.user_id, item.task.task_name) : null}/>)}</div>{item.excluded?.length ? <div className="excluded-box">{item.excluded.map(row => <p key={row.user_id}>{row.name}：{row.reason}</p>)}</div> : null}</section>)}
  </>
}
