...
function RecommendCard({ item, selected, onSelect, onAccept }) {
  ...
  return <div className={`recommend-item ${selected ? 'selected' : ''}`} onClick={() => onSelect(item.user_id)}>
    ...
    <div className="recommend-main">
      ...
      <div className="reason-chips">{(dims.skill?.skill_families || []).slice(0, 3).map(family => <span key={family}>{family}</span>)}{dims.quality?.samples > 0 && <span>质量样本 {dims.quality.samples} 条</span>}{dims.efficiency?.samples > 0 && <span>工时样本 {dims.efficiency.samples} 条</span>}{dims.load?.current_load ? <span>负载 {dims.load.current_load}</span> : null}<span className={`source-chip ${item.source !== 'rule' ? 'ai' : ''}`}>{sourceLabel(item.source || dims.skill?.source)}</span></div>
