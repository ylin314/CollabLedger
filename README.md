...
更新时间：2026-08-28。分支约定：`main` 只合可运行代码；rxc 当前开发分支为 `dev_D`。角色对应：A=ly，B=dkd，C=czc，D=rxc。任务编号（A1/B1/C1/D1）保持不变。
...
| 阶段四 长期协作 P2 | 历史项目、画像、跨项目授权 | 收尾交付 | D6 成员长期画像已交付：跨项目聚合技能/质量/效率/贡献画像接口 + 推荐器样本不足时历史兜底 + 前端画像面板；D7 已收尾：CI + 80 用例 + 演示手册；跨项目授权 / 归档只读仍为 TODO |
...
- [x] D7：新增 GitHub Actions CI（后端 pytest + 前端 build）、补齐接口/安全/边界测试至 80 用例、新增 docs/演示手册.md
