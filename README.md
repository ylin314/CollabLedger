...
| 阶段四 长期协作 P2 | 历史项目、班级成员池、履历、画像与跨项目授权 | 部分完成               | 历史项目、动态班级和同班成员履历已完成；D6 长期画像已交付（跨项目聚合技能/质量/效率/贡献接口 + 推荐器样本不足时历史画像兜底 + 前端画像面板）；D7 已收尾：CI + 80 用例 + 演示手册；跨项目授权 / 归档只读仍为 TODO |
...
- [ ] C11：历史项目页面、班级成员页、同班成员履历查看和个人头像管理已完成；仍待补充画像计算和更细粒度数据授权，不使用假数据
